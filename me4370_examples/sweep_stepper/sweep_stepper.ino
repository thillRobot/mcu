...
// Goal: integrate with 'rplidar_3d' ROS node - in progress, getting closer
...
void command_steps(int steps, bool dir)
{
  ...
  {   
    ...
    while(step_flag==0){            // wait until steps are complete (blocking)   
      pos_msg.data = motor_pos;      // publish pos to ROS
      pos_pub.publish( &pos_msg );
      nh.spinOnce();
      delay(.001);
    }
...
ISR(TIMER1_OVF_vect) 
{
  ...
  {   
    PORTA=halfstep[7-seq_cnt];   // step backward
    motor_pos--;   
  }

  step_cnt++;
    
  if(step_cnt==step_cmd) // finished last step 
  {
    step_flag=1;
    TIMSK1 &= 0b11111110; // turn off interrupt 
  } 
